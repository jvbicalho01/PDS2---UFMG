...
// version table: MSVC++ 15.0 _MSC_VER == 1910 (Visual Studio 2017) MSVC++ 14.0
// _MSC_VER == 1900 (Visual Studio 2015) MSVC++ 12.0 _MSC_VER == 1800 (Visual
// Studio 2013) MSVC++ 11.0 _MSC_VER == 1700 (Visual Studio 2012) MSVC++ 10.0
// _MSC_VER == 1600 (Visual Studio 2010) MSVC++ 9.0  _MSC_VER == 1500 (Visual
// Studio 2008) MSVC++ 8.0  _MSC_VER == 1400 (Visual Studio 2005)
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
  ...
  // for now... https://github.com/philsquared/Catch/issues/870
...
struct DOCTEST_INTERFACE CurrentTestCaseStats {
  ...
  String error_string;
  bool should_reenter;  // means we are not done with the test case because of
                        // subcases
...
struct DOCTEST_INTERFACE IReporter {
  ...
  // called when a test case has ended - could be re-entered if more subcases
  // have to be traversed - check CurrentTestCaseStats::should_reenter (caching
  // a pointer to the input doesn't make sense here)
...
#define DOCTEST_TEST_CASE_TEMPLATE_DEFINE_IMPL(dec, T, id, anon)   \
  ...
  struct DOCTEST_CAT(id, ITERATOR) {                               \
    DOCTEST_CAT(id, ITERATOR)(int line, int index) {               \
      DOCTEST_REGISTER_TYPED_TEST_CASE_IMPL(anon<Type>, Type, dec, \
                                            line * 1000 + index);  \
...
#define DOCTEST_TEST_CASE_TEMPLATE_DEFINE(dec, T, id) \
  DOCTEST_TEST_CASE_TEMPLATE_DEFINE_IMPL_PROXY(       \
      dec, T, id, DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))

#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, anon, ...)     \
  DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_CAT(anon, DUMMY)) = []() {        \
    DOCTEST_CAT(id, ITERATOR)<__VA_ARGS__> DOCTEST_UNUSED DOCTEST_CAT( \
...
DOCTEST_THREAD_LOCAL bool
    g_no_colors;  // used to avoid locks for the debug output
...
struct ConsoleReporter : public IReporter {
  ...
  void test_case_end(const CurrentTestCaseStats& st) override {
    ...
    if (crashed || (st.failure_flags & TestCaseFailureReason::Exception)) {
      ...
      s << Color::Red
        << (crashed ? "test case CRASHED: " : "test case THREW exception: ")
    ...
    // means the test case will be re-entered because there are untraversed (but
    // discovered) subcases
    ...
    if (rb.m_at &
      ...
      s << (rb.m_threw ? "threw as expected!" : "did NOT throw at all!")
        << "\n";
               ...
               assertType::is_throws_as) {  //! OCLINT bitwise operator in
                                            //! conditional
      s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", "
...
// extension of the console reporter - with a bunch of helpers for the stdout
// stream redirection
struct ConsoleReporterWithHelpers : public ConsoleReporter {
  ...
  void output_c_string_with_newline(const char* str) {
    s << Color::None << str << "\n";
  }
};
...
#define DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(func, type) \
  ...
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(test_run_start,
                                                const ContextOptions&)
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(test_run_end,
                                                const TestRunStats&)
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(test_case_start,
                                                const TestCaseData&)
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(test_case_end,
                                                const CurrentTestCaseStats&)
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(subcase_start,
                                                const SubcaseSignature&)
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(subcase_end,
                                                const SubcaseSignature&)
  ...
  DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(test_case_skipped,
                                                const TestCaseData&)
};
...
bool parseOption(int argc,
  ...
  if (parseOptionImpl(argc, argv,
                      pattern + strlen(DOCTEST_CONFIG_OPTIONS_PREFIX), res))
    return true;
#endif  // DOCTEST_CONFIG_NO_UNPREFIXED_OPTIONS
...
bool parseIntOption(int argc,
  ...
  if (type == 0) {
    ...
    // undefined behavior - assumes '0' on unsuccessful parse...
...
int Context::run() {
  ...
  if (p->filters[8].empty())
    p->reporters_currently_used.push_back(
        getReporters()[reporterMap::key_type(0, "console")]);
  ...
  for (auto& curr : testArray) {
    ...
    // execute the test if it passes all the filtering
    {
      ...
      do {
        ...
        p->numAssertsForCurrentTestCase +=
            p->numAssertsForCurrentTestCase_atomic;
        ...
        // call it from here only if we will continue looping for other subcases
        // and call it again outside of the loop for one final time - with
        // updated flags
        if (p->should_reenter == true) {
          DOCTEST_ITERATE_THROUGH_REPORTERS(test_case_end, *g_cs);
          // remove these flags - it is expected that the reporters have handled
          // these issues
          p->failure_flags &= ~TestCaseFailureReason::Exception;
          p->failure_flags &= ~TestCaseFailureReason::AssertFailure;
        }
      ...
      if (Approx(p->currentTest->m_timeout).epsilon(DBL_EPSILON) != 0 &&
          Approx(p->seconds_so_far).epsilon(DBL_EPSILON) >
      ...
      bool ok_to_fail =
          (TestCaseFailureReason::ShouldHaveFailedAndDid & p->failure_flags) ||
...
int registerReporter(const char* name, int priority, IReporter& r) {
  getReporters().insert(
      reporterMap::value_type(reporterMap::key_type(priority, name), &r));
  return 0;
}
